   ...
   const cadastrar = ()=>{
                ...
                search.map(async (data)=>{
                    if(data.permissao=='Recusar'){
                       setDanger('Acesso negado para este usuário!')
                        ...
                        localStorage.setItem('alunoData', dadoJSON)
                        setDanger('Acesso Permitido')
                        setNext('/aluno/home')
                       
                        await axios.get('http://localhost:5000/coordenador')
                        .then((response)=>{
                           const value = response.data.filter((data)=>data.area === dado.area)
                           localStorage.setItem('coordenador',JSON.stringify(value))
                           alert(value)
                        })
    ...
    return(
        <>
            ...
            {senhaErr && <small>A senha deve incluir:
