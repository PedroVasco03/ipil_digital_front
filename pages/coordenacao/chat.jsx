...
function ChatCoordenacao(){
    ...
    const getMensagem = async ()=>{
        ...
        .then((response)=>{
            let dado = JSON.parse(localStorage.getItem('coordenadorData'))
            const search = response.data.filter((data)=> dado.id === data.idEnviar)
            setMensagem(search)
        })
        ...
        <div className={styleGeral.container}>
            ...
            {/* <div className={styleGeral.content}>
                ...
                {mensagem.map((item, index) => {
                    return (
                        <Card className="m-4 p-3 w-75">
                            <CardBody>
                                <div>
                                    <h5>{item.nomeEnviado}</h5>
                                    <p className="m-3 text-muted">Mensagem: {item.mensagem}</p>
                                </div>
                                <div style={{ float: 'right' }}>
                                    <label style={{ cursor: 'pointer' }} className="text-danger p-2">eliminar mensagem</label>
                                </div>
                            </CardBody>
                        </Card>
                    );
