...
function SettingsEncarregado(){
    ...
    const getEncarregado= async ()=>{
        const data = await JSON.parse(localStorage.getItem('encarregadoData'))
        setEncarregado(data)
    ...
    const deleteAluno = async ()=>{
        const data = await JSON.parse(localStorage.getItem('encarregadoData'))
        await axios.delete(`http://localhost:5000/encarregado/${data.id}`)
    }
                   ...
                   <Card className="m-2">
                        <CardBody>
                            ...
                            <div className="d-flex">
                                <Link href={'/login/encarregado/login'}>
                                    <Button color="danger" className="m-2" onClick={()=>{
                                        deleteAluno()
                                        localStorage.removeItem("encarregadoData")
